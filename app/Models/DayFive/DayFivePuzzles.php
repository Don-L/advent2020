...
/*
use \App\Models\DayFive\DayFivePuzzles;
$puzzles = new DayFivePuzzles();
$puzzles->dayFiveStarTwo();
$puzzles->dayFiveStarOneReal();
$puzzles->dayFiveStarOneTest1();
$puzzles->dayFiveStarOneTest2();
$puzzles->dayFiveStarOneTest3();
$puzzles->dayFiveStarOneTest4();
 */

class DayFivePuzzles
{
    public function dayFiveStarTwo(): int
    {
        $passes   = new BoardingPassService(['rows' => 128, 'cols' => 8]);
        $seat_ids = [];
        foreach($this->real_data as $seat_string) {
            $pass       = $passes->createNewPassFromString($seat_string);
            $seat_ids[] = $passes->seatIdForPass($pass);
        }
        rsort($seat_ids);
        $missing;
        foreach ($seat_ids as $i => $id) {
            if ($id - $seat_ids[$i + 1] > 1) {
                $missing = $id - 1;
                break;
            }
        }
